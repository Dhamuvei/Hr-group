...
module.exports.addIpAddress = async(req,res)=>{
    try {
        let newCity = new ipAddressModel({
            name : req.body.name,
...
module.exports.getIpAddress = async (req, res) => {
    try {
      ...
      const ipAddressDetails = await ipAddressModel.find(filterObj)
    //     .populate({
    //       path: 'countryId',
    //       select: ['countryName'],
    //   })
    ...
    //   })
        ...
        responseHandlier.successResponse(true, ipAddressDetails, res);
    } catch (error) {
